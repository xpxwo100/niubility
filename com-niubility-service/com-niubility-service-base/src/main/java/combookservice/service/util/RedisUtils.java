...
public class RedisUtils {
    ...
    public GeoResults<RedisGeoCommands.GeoLocation<Object>> getRadiusByPoint(String key, double lon, double lat, int radius, RedisGeoCommands.DistanceUnit unit, int limit) {
        try {
            Circle circle = new Circle(new Point(lon, lat), new Distance(radius, unit));
            RedisGeoCommands.GeoRadiusCommandArgs args;
            if (limit == -1) {
                args =  RedisGeoCommands.GeoRadiusCommandArgs.newGeoRadiusArgs().includeDistance().includeCoordinates().sortAscending();
            }else{
